...
```shell
# Dapp address
DAPP_ADDRESS=0xF8C694fd58360De278d5fF2276B7130Bfdc0192A
...
```shell
curl -s -H 'Content-Type: application/json' -X POST http://localhost:4000/graphql -d '{"query": "query { reports { nodes { payload }}}"}' | jq -r '.data.reports.nodes[] | .payload,"0a0a"' | xxd -r -p
```
...
contract Score {
    ...
    function addScore(bytes calldata _payload, OutputValidityProof calldata _v) public {
        IOutput dapp = IOutput(dappAddress);

        // validate notice
        dapp.validateNotice(_payload,_v);

        // check if notice has been processed
        uint256 noticePosition = getBitMaskPosition(_v.outputIndex,_v.inputIndex,_v.epochIndex);
    ...
    function getBitMaskPosition(uint256 _notice,uint256 _input,uint256 _epoch) public pure returns (uint256) {
        return (((_notice << 128) | (_input << 64)) | _epoch);
    }
...
cast send $CONTRACT_ADDRESS \
    "addScore(bytes,(uint256,uint256,uint256,bytes32,bytes32,bytes32,bytes32,bytes32[],bytes32[]))" $NOTICE_RESULT \
    --rpc-url http://localhost:8545 --from $PLAYER1 --private-key $PLAYER1_KEY
```
